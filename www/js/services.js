angular.module('app.services', [])
		...
		factory('Users', function ($cordovaSQLite, DBA, $filter, $http) {
            ...
            self.add = function (data) {
				//console.log("data = "+data.ID,data.username,data.password,data.complete_name,data.status,data.last_sync);
                var parameters = [data.ID,data.username,data.password,data.complete_name,data.status,data.last_sync];
		...
		.factory('Book', function (DBA, $http,$rootScope,$localStorage,$ionicLoading) {
            ...
            self.updateSync = function () {
				...
				$http.get('//localhost/?getBooking='+userId).success(function (data) {
			...
			self.add = function (data) {
                ...
                var parameters = [data.ID,data.location,data.cv_number,data.account_name,data.book_a,data.book_k,
				data.book_foc,data.book_inf,data.book_tg,data.book_e,data.book_arrival,data.book_time,
				data.actual_a,data.actual_k,data.actual_foc,data.actual_inf,data.actual_tg,data.actual_e,
				data.actual_arrival,data.actual_time,data.resort_hotel,data.unit,data.unit_atd,data.driver,
				data.coordinator,data.sales_handle,data.agency,data.remarks,data.user,data.status]
				
				
                return DBA.query("INSERT INTO book (ID, location, cv_number, account_name, book_a, book_k, book_foc, book_inf, book_tg, book_e, book_arrival, book_time, actual_a, actual_k, actual_foc, actual_inf, actual_tg, actual_e, actual_arrival, actual_time, resort_hotel, unit, unit_atd, driver, coordinator, sales_handle, agency, remarks, user,status )\n\
                                                         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", parameters);
            }
			self.removeALL = function () {
                var parameters = [];
